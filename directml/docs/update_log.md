...
# M5 --- Final DirectML Quality Assessment

-   Re-evaluated the final locally trained Dense pretraining checkpoints after
    the execution, loss, and optimizer fixes.
-   Confirmed that the tokenizer encodes/decodes correctly and that the
    `pretrain_t2t_mini.jsonl` dataset is readable and coherent.
-   Evaluated the official upstream `pretrain_768.pth` checkpoint with the same
    `eval_llm.py` path.
-   Confirmed coherent generation from the official checkpoint on both CPU and
    `directml:1`.
-   Confirmed that the locally trained DirectML epoch-1 checkpoint is
    incoherent.
-   Confirmed that the locally trained DirectML epoch-2 checkpoint is also
    incoherent.
-   Ruled out the tokenizer, dataset, checkpoint loader, and DirectML inference
    path as the primary explanation for the quality gap.
-   Concluded that the custom DirectML training path did not produce acceptable
    model quality despite finite losses, matching diagnostic losses, and
    successful full-epoch execution.
-   Reclassified the previous M4/M5 “practically viable” DirectML conclusion as
    an execution/performance result only, not a final model-quality validation.
-   Stopped further DirectML full-training work.
-   Preserved all DirectML compatibility code, benchmarks, tests, issues, and
